...
> **状态：** 已实现并接入主场景（2026-07 起）  
> **时间节点：** 2026-07-14 定稿；主路径接入后本文 §0–§1 公式仍有效  
> **范围：** 运动核 + 沙盒 + 参数面板 + **主游戏卡牌位移/拖拽**  

> **当前架构摘要**  
> 1. 公式与选型仍以本文 §0–§1 为准（禁止 Verlet 多段绳 / Tween 模拟过冲）。  
> 2. 主场景：`CardView` 默认 `positionDriver=external`，内嵌 `ElasticRopeMotion`；流程只 `setMoveTarget`，过冲由绳产生。  
> 3. 沙盒 `?scene=elastic-rope` 与主场景共用同一驱动。  
> 4. 旧 `cardOvershoot` / lerp 位移路径已退役（配置与面板可保留兼容）。
...
## 10. 主路径接入后仍可选的后续项（非阻塞）

- 参数面板对已退役 `cardOvershoot` / 部分 `dragHandCard` 位移字段做 UI 灰显标注  
- 按场景（手牌 / 出牌 / 弃牌）为绳参加 profile 倍率  
- 路径点队列 `route[]`  
- 引入 npm 物理引擎依赖（**不建议**，与本文选型冲突）
...
2. `/?scene=elastic-rope`：单卡可拖、松手冻结目标、绳 debug 可选。  
3. 无 query 主游戏：拖拽 / 归位 / 发牌 / 出牌 / 弃牌 / 理牌均走弹性绳目标点模型。  
4. 面板「弹性绳子牵引卡牌模型」参数影响主场景与沙盒。  
